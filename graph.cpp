...
private:
    ...
    // Helper functions
    void addEdge(const std::string& from, const std::string& to, int weight, const std::string& type);
    ...
    double calculateMovieSimilarity(const std::string& movie1, const std::string& movie2);
...
public:
    ...
    void removeRating(const std::string& userId, const std::string& movieId);
    ...
    std::vector<Movie> getHybridRecommendations(const std::string& userId, 
                                               int maxRecommendations = 10);
    std::vector<Movie> getSimilarMovies(const std::string& movieId, int maxResults = 5);
    ...
    std::vector<std::string> getUserTopGenres(const std::string& userId, int count = 3);
    std::vector<std::string> getUserRecommendationHistory(const std::string& userId);
    ...
    // Data persistence
    bool exportGraphToFile(const std::string& filename);
    bool importGraphFromFile(const std::string& filename);
    ...
    const std::unordered_map<std::string, std::vector<Edge>>& getAdjacencyList() const { return adjacencyList; }
};
